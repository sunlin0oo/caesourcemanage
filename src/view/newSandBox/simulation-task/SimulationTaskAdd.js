...
function SimulationTaskAdd(props) {
  // 记录网格的模型名称
  ...
  const handleSave = (auditState) => {
    axios.post(`/simulationtasks`, {
      ...
      "gridstaskId": Number(nameInfo.key)
    }).then(res => {
    ...
    <div>
      ...
      <div className={current === 2 ? '' : 'active'}>
        ...
        <Descriptions title="最终内容" bordered>
          <Descriptions.Item label="模型名称">{nameInfo.label}</Descriptions.Item>
...
export default WithRouter(SimulationTaskAdd)